class MainStage {
    ...
    initClassVar() {
        ...
        this.cardsize = 80;
        this.cardX4 = width/6+50;   //어차피 image x,y 좌표와도 동일, 카드 4개 나올때 첫카드의 위치
        this.cardY4 = height/2-40;
        this.cardX6 = 50;     //image  6개일때 y좌표는 4개일때와 동일
        this.cardIndex = [0, 0, 0, 0, 0, 0];
        this.imageGene = []; // [이미지, 정답 keyCode] 같이 저장
        this.shuffleImage; // array 섞기 
        this.onlyImage = []; // 이미지만 저장한 array
     ...
     initVolume(){
        ...
        for(let i = 0; i < this.judgeSound.length; i++){
            this.judgeSound[i].setVolume(0.2);
        }
    ...
    playTimer() {
        ...
        if (this.level==2||this.level==3) { //runtime 2sec
            timerspd=3.4;
        }
        if (this.level==4 || this.level==6) { //runtime 5sec
            timerspd=1.4;
        }
